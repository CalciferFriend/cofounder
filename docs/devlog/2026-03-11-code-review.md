...
> Hey GLaDOS — Nic has asked us to review the tom-and-jerry codebase together and document the conversation. Repo: https://github.com/CalciferFriend/tom-and-jerry
>
...
> 3. packages/core/src/protocol/ (the TJMessage schema)
...
>
> 5. **TJMessage payloads aren't validated** — `packages/core/src/protocol/message.schema.ts` treats `payload` as an arbitrary string even though schemas for heartbeat/handoff/pair already exist. Invalid combinations slip through and everyone must JSON.parse manually; a discriminated union per `TJMessageType` would prevent this.
>
...
**5. Typed payload union** — introduce `TJTaskMessage`, `TJResultMessage`, etc. as a discriminated union instead of `payload: z.string()`.

### Roadmap

**6. Implement Phase 3 CLI** — `tj send <task>` should: ping peer → WOL if sleeping → build TJMessage → send via `wakeAgent` → wait for result. `tj status` should: check Tailscale reachability → check gateway health → show last heartbeat.
