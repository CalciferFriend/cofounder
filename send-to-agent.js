#!/usr/bin/env node
/**
 * send-to-agent.js — Agent-to-agent messaging script for tom-and-jerry
 *
 * Usage:
 *   node send-to-agent.js --url ws://100.x.x.x:18789 --token <token> --message "text"
 *   node send-to-agent.js --config ~/.tom-and-jerry/config.json --to glados --message "text"
 *
 * Options:
 *   --url      WebSocket URL of target gateway (ws://host:port)
 *   --token    Gateway auth token (or set TJ_PEER_TOKEN env var)
 *   --to       Peer role to message (tom|jerry) — resolves from config
 *   --config   Path to tom-and-jerry config.json (default: ~/.tom-and-jerry/config.json)
 ...
 *   node send-to-agent.js --to jerry \
...
const tokenArg = getArg(["--token"]) ?? process.env.TJ_PEER_TOKEN ?? null;
const toPeer = getArg(["--to"]);
const configPath = getArg(["--config"]) ?? join(homedir(), ".tom-and-jerry", "config.json");
...
if (!resolvedToken) {
  err("[send-to-agent] --token is required (or set TJ_PEER_TOKEN env var)");
  process.exit(2);
}
