...
hh result <id> --json '{"output":"...","artifacts":["/tmp/cat.png"]}'
```

`<id>` is the task UUID (from `hh logs` or the `HH_TASK_ID` env var injected by `hh watch`).
...
`hh result` writes a `TaskResult` to `~/.his-and-hers/state/tasks/<id>.json`. H1's `hh send --wait` polling loop reads this file and resolves when the status changes.

When `--webhook-url` is supplied (typically embedded automatically in the H1 wake message), the result is also POSTed to H1 directly — resolving `hh send --wait` without any polling delay and triggering any configured notification webhook.

## Examples

```bash
# Standard result: task completed successfully
hh result abc-1234 "Image saved to /tmp/cat.png"

# Task failed
hh result abc-1234 --fail "Ollama model not available: llama3.2"

# Read large output from a file
hh result abc-1234 --output-file /tmp/transcription.txt

# Full structured result with artifacts and cost metadata
hh result abc-1234 --json '{
...
# With webhook delivery (URL injected by hh send):
hh result abc-1234 "done" \
  --webhook-url http://100.x.x.x:38791/result
```

## Executor integration

When H2 runs `hh watch --exec <cmd>`, the executor is launched with `HH_TASK_ID`, `HH_TASK_OBJECTIVE`, and `HH_TASK_FROM` set as environment variables. The typical pattern:
...
```bash
ssh glados "hh result <id> 'task done'"
```
...
- [`hh watch`](/reference/watch) — H2 daemon that auto-dispatches tasks
- [`hh logs`](/reference/logs) — view task history and statuses
- [`hh replay`](/reference/replay) — re-send a previous task
